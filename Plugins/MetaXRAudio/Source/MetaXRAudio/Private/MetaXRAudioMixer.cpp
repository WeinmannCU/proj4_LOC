...
TRACE_DECLARE_INT_COUNTER(COUNTER_METAXRAUDIO_NumSpatializers, TEXT("MetaXR/Audio/NumSpatializers"));
TRACE_DECLARE_INT_COUNTER(COUNTER_METAXRAUDIO_NumMonoSources, TEXT("MetaXR/Audio/NumMonoSources"));

MetaXRAudioSpatializationAudioMixer::MetaXRAudioSpatializationAudioMixer() : Context(nullptr) {
  TRACE_COUNTER_INCREMENT(COUNTER_METAXRAUDIO_NumSpatializers);
}

MetaXRAudioSpatializationAudioMixer::~MetaXRAudioSpatializationAudioMixer() {
  TRACE_COUNTER_DECREMENT(COUNTER_METAXRAUDIO_NumSpatializers);
}
...
void MetaXRAudioSpatializationAudioMixer::Initialize(const FAudioPluginInitializationParams InitializationParams) {
  TRACE_CPUPROFILER_EVENT_SCOPE_STR("MetaXRAudioSpatializationAudioMixer::Initialize");
  ...
  if (!Context) {
    ...
    if (Context == nullptr) {
      UE_LOG(LogAudio, Error, TEXT("Failed to create Meta XR Audio Context during Initialization of Mixer"));
    }
  }
...
void MetaXRAudioSpatializationAudioMixer::OnInitSource(
  ...
  TRACE_CPUPROFILER_EVENT_SCOPE_STR("MetaXRAudioSpatializationAudioMixer::OnInitSource");
  ...
  TRACE_COUNTER_INCREMENT(COUNTER_METAXRAUDIO_NumMonoSources);
}

void MetaXRAudioSpatializationAudioMixer::OnReleaseSource(const uint32 SourceId) {
  TRACE_COUNTER_DECREMENT(COUNTER_METAXRAUDIO_NumMonoSources);
}
...
void MetaXRAudioSpatializationAudioMixer::ProcessAudio(
  ...
  TRACE_CPUPROFILER_EVENT_SCOPE_STR("MetaXRAudioSpatializationAudioMixer::ProcessAudio");
  ...
  if (InputData.SpatializationParams && Context) {
    ...
    if (SourceSettings[InputData.SourceId] != nullptr) {
      ...
      SpatializationEnabled = SourceSettings[InputData.SourceId]->bEnableSpatialization;
    }
...
bool MetaXRAudioSpatializationAudioMixer::Tick(float DeltaTime) {
  TRACE_CPUPROFILER_EVENT_SCOPE_STR("MetaXRAudioSpatializationAudioMixer::Tick");
