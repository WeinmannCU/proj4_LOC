...
TRACE_DECLARE_INT_COUNTER(COUNTER_METAXRAUDIO_NumReverbPlugins, TEXT("MetaXR/Audio/NumReverbPlugins"));
...
void FSubmixEffectMetaXRReverbPlugin::OnNewDeviceCreated(Audio::FDeviceId DeviceId) {
  TRACE_CPUPROFILER_EVENT_SCOPE_STR("FSubmixEffectMetaXRReverbPlugin::OnNewDeviceCreated");
...
void FSubmixEffectMetaXRReverbPlugin::OnNewDeviceDestroyed(Audio::FDeviceId DeviceId) {
  ClearContext();
}
...
void FSubmixEffectMetaXRReverbPlugin::Init(const FSoundEffectSubmixInitData& InInitData) {
  ...
  DeviceDestroyedHandle =
      FAudioDeviceManagerDelegates::OnAudioDeviceDestroyed.AddRaw(this, &FSubmixEffectMetaXRReverbPlugin::OnNewDeviceDestroyed);
}

FSubmixEffectMetaXRReverbPlugin::FSubmixEffectMetaXRReverbPlugin() : Context(nullptr) {
  TRACE_COUNTER_INCREMENT(COUNTER_METAXRAUDIO_NumReverbPlugins);
}
...
FSubmixEffectMetaXRReverbPlugin::~FSubmixEffectMetaXRReverbPlugin() {
  ...
  TRACE_COUNTER_DECREMENT(COUNTER_METAXRAUDIO_NumReverbPlugins);
}
...
void FSubmixEffectMetaXRReverbPlugin::OnProcessAudio(
  ...
  TRACE_CPUPROFILER_EVENT_SCOPE_STR("FSubmixEffectMetaXRReverbPlugin::OnProcessAudio");
  ...
  if (currentContext) {
    ...
    if (Settings != nullptr) {
      ...
      Result = OVRA_CALL(ovrAudio_Enable)(currentContext, ovrAudioEnable_EarlyReflections, Settings->bEarlyReflectionsEnabled);
      OVR_AUDIO_CHECK(Result, "Failed to enable reflections");

      Result = OVRA_CALL(ovrAudio_Enable)(currentContext, ovrAudioEnable_LateReverberation, Settings->bReverbEnabled);
    ...
    if (AcousticProjectSettings != nullptr) {
      Result = OVRA_CALL(ovrAudio_Enable)(currentContext, ovrAudioEnable_Diffraction, AcousticProjectSettings->bDiffractionEnabled);
      OVR_AUDIO_CHECK(Result, "Failed to set diffraction enabled");

      Result =
          OVRA_CALL(ovrAudio_SetAcousticModel)(currentContext, static_cast<ovrAudioAcousticModel>(AcousticProjectSettings->AcousticModel));
...
USoundSubmix* MetaXRAudioReverb::GetSubmix() {
  ...
  if (Settings == nullptr) {
    USoundSubmix* StubSubmix = NewObject<USoundSubmix>(USoundSubmix::StaticClass(), TEXT("Meta XR Audio Stub Submix"));
    StubSubmix->bMuteWhenBackgrounded = true;
    StubSubmix->bAutoDisable = false;
    UE_LOG(LogAudio, Error, TEXT("Failed to get Meta XR Audio settings, creating Meta XR Audio Stub Submix as a stub"));
    return StubSubmix;
  }
