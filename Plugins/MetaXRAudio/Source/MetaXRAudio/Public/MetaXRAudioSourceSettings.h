...
class METAXRAUDIO_API UMetaXRAudioSourceSettings : public USpatializationPluginSourceSettingsBase {
 ...
 public:
  ...
  // If enabled, the Meta XR Audio plugin will apply distance attenuation to the sound. This should only be used when the native distance
  // attenuation is disabled
  UPROPERTY(GlobalConfig, BlueprintReadWrite, EditAnywhere, Category = "SpatializationSettings")
  bool bEnableMetaDistanceAttenuation;
  ...
  // Adjust how much occlusion to apply with 1 being complete occlusion and 0 being no occlusion
  UPROPERTY(
      GlobalConfig,
      BlueprintReadWrite,
      EditAnywhere,
  ...
  // If enabled and using Acoustic Ray Tracing, medium absorption will be applied to the source over distance
  UPROPERTY(GlobalConfig, BlueprintReadWrite, EditAnywhere, Category = "ExperimentalSpatializationSettings")
  bool bMediumAbsorption;
};
